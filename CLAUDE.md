...
- `AI_MOCK_DATA.md` — **honest audit doc**; its Priority-1 fix was never applied (`/api/ai/performance/predict` and `/api/ai/tone/analyze` still import `simpleAIService`, bypassing the mock-fallback layer — ADR-0018 scope). ~~All six social providers still return `generateMockAnalytics()`~~ — **RESOLVED (ADR-0009).**
