...
| Admin RBAC (`/api/admin/users\|teams\|sso/**`) | ✅ **Authorization repaired (ADR-0004, 2026-07-03):** two-tier model — `User.isPlatformAdmin` gates all cross-tenant `/api/admin/**` + global settings via `requirePlatformAdmin()`; `WorkspaceRole` via `requireWorkspaceRole()`. Cross-tenant escalation closed (workspace OWNER now 403, verified); `Role`/`UserRole`/`Permission` tables + `/api/admin/roles\|permissions` routes/pages deleted. Remaining route↔schema divergences (bulk-ops, sso shape) still ADR-0012 | Root admin page still hardcoded; user modals stubs; static roles-reference page replaces the permission matrix | ⚠️ Authz works; full CRUD repair is ADR-0012 |
...
- Still open (other ADRs): `/api/support/tickets/[ticketId]` GET/PUT/updates/attachments allow anyone with a ticket ID to read/modify/upload (ADR-0011); help/docs/video write endpoints unauthenticated `TODO` (ADR-0014/0024).
