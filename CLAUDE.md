...
5. ✅ **RESOLVED 2026-07-06 (ADR-0017 implemented)** — ~~User settings UI is disconnected~~. `SettingsProvider` + a next-themes `ThemeProvider` are now mounted in `providers.tsx`; `/dashboard/settings` is rewritten onto `useSettings()` (real Save/Export against the DB APIs), `/dashboard/profile` shows real data (no fabricated stats), settings-PUT is value-validated (ADR-0005), GDPR `POST /api/user/export` + `DELETE /api/user/account` (re-auth + sole-owner guard) ship, i18n is cut to `en` (runtime OpenAI MT removed), and the mock `/dashboard/workspace` switcher is deleted. Backend verified live (validation 400s, export no-password, deletion guard chain 400→401→409) AND **browser-render verified on a host dev server** (settings loads real data, theme applies instantly app-wide via next-themes, profile shows real `createdAt`) — which caught + fixed a real bug the static gate missed (profile imported the uninstalled `sonner` package → webpack Build Error; commit `bb96207` switched it to the in-repo `useToast`). **Note:** 2FA deferred to ADR-0026 (dead toggle removed); real workspace switching to ADR-0027. To browser-test, use a HOST `next dev` (the docker app hits the pre-existing `next-auth/react` chunk quirk, ADR-0006 note); this repo's tsc does NOT reliably flag missing-package imports, so a real build/render check is required.
...
| AI features (`/api/ai/**`, `/api/automation/**`, `/api/audience/**`) | ✅ **Honest (ADR-0018, 2026-07-07):** one availability contract — real key → `openai`; no key + explicit demo mode → `mock` with `simulated:true` stamped; otherwise **503 `AI_UNAVAILABLE`** (the silent Math.random fallback, the `sk-fake-key` 500s, and the import crash are all gone). Env model policy (`OPENAI_MODEL`), one cost table, usage tracking on every caller, Redis AI cache, `GET /api/ai/status`. Plan-credit 402s (ADR-0019) + optional `AI_MONTHLY_COST_LIMIT_CENTS` 429 backstop. | **Mounted**: `/dashboard/audience` (3 honest tabs — the fabricated Intelligence dashboard was deleted), analytics Visual Insights tab, composer AI toggles availability-gated with demo badges | ✅ Yes (unit+integration+browser verified) |
...
| Billing (`/dashboard/billing`) | ✅ **Real (ADR-0019, 2026-07-07):** Stripe Checkout+Portal routes, signature-verified idempotent webhook (`StripeEvent` ledger), `GET /api/billing/subscription` (entitlements+usage+real invoices); server-side limit enforcement (accounts/posts/seats/AI credits → 402 `limit_exceeded`) via one entitlements helper; 14-day no-card PRO trial at signup (proven live). **No mock fallback** — without `STRIPE_SECRET_KEY` checkout/portal return honest 503. | Rebuilt on live data: tier+trial countdown, usage meters, plan cards, PAST_DUE banner, real invoice list — mock plans/mockInvoices/VISA-4242 **deleted** | ✅ Yes (live checkout redirect needs real Stripe keys) |
| Client portal & report sharing (`/share/reports/[token]`, `/portal`, `/api/share\|portal/**`) | ✅ **Real (ADR-0020 Phases 0–2, 2026-07-07):** `ReportShareLink` tokenized snapshot links (sha256-at-rest tokens shown once, optional bcrypt password + HMAC cookie proof, expiry/revocation, uniform 404 — no oracle, snapshot-only render, share URL embeddable in report emails); `CLIENT_VIEWER` finally real — `UserWorkspace.clientId` scope, `requireClientViewer()`, exhaustive read-only allowlist (list/detail/download scoped; the download route's any-workspace/no-role hole is closed for ALL roles) + **JWT `portalOnly` edge default-deny** (~175 legacy membership-only routes 403 `PORTAL_ONLY` for portal contacts); portal invites PRO+ (`assertPlanFeature('clientPortal')` → 402), share links ungated; honest labels (`*_Report.html`, "Printable report"); fictional `client-permission-system.tsx` + phantom `ClientRole` enums deleted | Share dialog in client-reports menus (one-time URL, revoke list); "Invite to portal" on client detail; 2-page branded portal + report detail reusing the share renderer | ✅ Yes — 66 new Jest tests, e2e g7 (share/revoke/portal/default-deny), live browser verified. Phase 3 (post approvals) separate go/no-go; real PDFs on the ADR-0008/0007 path |
...
- ✅ **CLIENT_VIEWER mis-permission (download-anything / list-nothing) — CLOSED (ADR-0020, 2026-07-07):** the role now has an exhaustive allowlist (client-scoped reports list/detail/download + `/api/portal/summary`); the report-download route's findFirst-any-workspace/no-role hole is fixed for every role; and portal-only users (all memberships CLIENT_VIEWER) are default-denied on every other `/api/*` by a `portalOnly` JWT claim enforced in the edge middleware — because ~175 legacy routes still check membership without role (incremental ADR-0004 rollout). e2e-proven (6 legacy routes 403 `PORTAL_ONLY`, allowlist 200).
...
- Demo login: **demo@sociallyhub.com** / the value of `DEMO_USER_PASSWORD` (dev compose sets `demo123456`), OWNER of `demo-workspace`. ADR-0025 D4: no committed constant passwords — the mock users get per-user random passwords (never printed), the demo password comes from env or is generated-and-printed-once. A blocking CI guard (`scripts/check-no-committed-demo-secrets.sh`) forbids `password123`/`demo123456` literals in source.
- ✅ **Demo mode + tiered seeding (ADR-0025, 2026-07-08):** demo is ONE explicit flag — `isDemoMode()` = `DEMO_MODE === 'true'` only (the `NODE_ENV==='development'` heuristic and the `ENABLE_DEMO` backdoor are **removed**; a credential-less prod install returns honest `PLATFORM_NOT_CONFIGURED` instead of fabricating accounts/AI). Gated behaviors are enumerated in `docs/demo-mode.md`. `prisma/seed.ts` is now a **tier dispatcher** (`--tier=` / `SEED_TIER`, default `minimal`): **minimal** = prod-safe idempotent bootstrap (settings defaults + `User.isPlatformAdmin` from `PLATFORM_ADMIN_EMAILS`, asserts an admin exists) via `src/lib/seeders/{settings-defaults,admin-user}-seeder.ts`; **demo** = requires `DEMO_MODE=true` (aborts otherwise), runs minimal then the ~30k showcase generator (wipe only under `--wipe`, preserves demo user + `demo-workspace` + a login-disabled `system` attribution user); **test** = deterministic, delegates to `seedE2E()`. The prod image esbuild-bundles the seed → `dist/seed.js` and the entrypoint runs `node dist/seed.js --tier=${SEED_TIER:-minimal}` after migrate (fail-loud). Dev compose sets `DEMO_MODE=true`/`SEED_TIER=demo`; prod does not. **Note:** the demo generator is kept cohesive (not atomized into 7 per-domain files — pragmatic parity choice).
- ✅ **Single Next config (ADR-0024, 2026-07-06):** `next.config.ts` deleted, its Docker watch-polling merged into `next.config.js` (guarded). **`next build` is GREEN (EXIT=0, 241 pages) for the first time in repo history** — eslint/typescript checks inside the build are intentionally deferred to their own CI stages (loud comments in the config; pre-existing lint/type backlogs are ADR-0021 ratchet scope; module-resolution/compilation — the class that caught the `sonner` incident — is fully enforced). Compose bind-mounts `package.json`/lock into app+worker so container installs see the current manifest.
- `src/middleware.ts` exists (edge gate: ADR-0005 no-store/request-id + ADR-0013/0014 feature-flag 404s). ~~`src/middleware/api-versioning.ts` mock demo routes~~ (deleted with the fake versioning layer, ADR-0024).
- ✅ **CI/CD real (ADR-0022, 2026-07-06):** honest 8-job `ci.yml` — **blocking**: schema (`db:check`+migrate+seed), **build** (`next build`+worker — the gate that would have caught the `sonner` incident), docker-image (GHCR `sha-<sha>` push on main); **advisory with measured justifications**: lint/typecheck/test/e2e/security (each flips to blocking by deleting one line when its ADR-0021 ratchet lands). `deploy.yml` = SSH tag-swap deploy with a body-asserting health gate (`"status":"healthy"`) + `.env.previous` auto-rollback; inert-but-loud until DEPLOY_* secrets + a VM exist (runbook: `docs/ops/deployment.md`). **Testing is REAL (ADR-0021, 2026-07-07): 12/12 Jest suites / 158 tests green** (root cause: the jest `projects[]` array bypassed next/jest wrapping — each project is now individually wrapped); coverage ratchet at the measured floor (global 1%, `encryption.ts` pinned 82/75/83/82; up-only rule); **the CI Jest job is BLOCKING** ('Jest (gate — ratcheted coverage)'). E2E: deterministic fixtures (`prisma/seed-e2e.ts` + `e2e/fixtures.ts`), golden paths proven in a live chromium session (signup→Mailhog→verify→signin, inbox, client report, billing — 17/17 specs incl. axe a11y over 4 pages); e2e CI job flips blocking after proving green against the prod image. Playwright honors `PLAYWRIGHT_BASE_URL`. Committed `.npmrc` (`legacy-peer-deps=true`) — plain `npm ci` fails without it.
- ✅ **Observability real (ADR-0023, 2026-07-07):** one `globalThis`-guarded **prom-client** registry (`src/lib/observability/metrics.ts`) — `/api/metrics` serves real *accumulating* HTTP counters (the old hand-rolled collector reset every counter to 1 per scrape) + DB gauges (`sociallyhub_{users,posts,workspaces}_total`), bearer-protected by `METRICS_TOKEN`, recorded through the single `withLogging` seam with bounded route-pattern labels; `src/instrumentation.ts` inits it once/process. `/api/health` is a **real readiness check** (singleton prisma `SELECT 1`, shared ioredis `PING`, worker-heartbeat gated by `WORKER_EXPECTED`, cwd/tmp write test deleted; degrades, never 500s unless the DB is down). **Every fabricated metric is gone:** `'99.9%'` uptime + both UI fallbacks, the monitoring dashboard's hardcoded 98/100/67/34% bars + `|| 127`, `/api/monitoring/metrics` + `/api/analytics/platform` `Math.random()` error/latency, `/api/clients/stats` revenue/industry/satisfaction/acquisition, `/api/campaigns/analytics` age/gender/location demographics — replaced with real sources or dropped, and a **blocking CI grep guard** (`scripts/check-no-fabricated-metrics.sh`, allowlist for ID-gen + other-ADR-owned routes) keeps them out. Prometheus config repaired + `promtool`-validated (invalid `recording_rules:` block → mounted `rules/`; targets trimmed to app/worker/self; Grafana unified alerting, no Alertmanager); Grafana datasources+dashboard provisioning; worker prom-client on internal `:9464`; `LOG_TO_STDOUT` strips winston file transports in containers. GlitchTip/`@sentry/nextjs` wired but **dormant without `SENTRY_DSN`** (guarded dynamic import, no `withSentryConfig` — keyless build is a no-op). **Gotcha:** the deps (`prom-client`, `@sentry/nextjs`) must exist in the container `node_modules` named volume — a stale volume 500s `/api/health`; fix is `docker compose exec app npm install` or `./dev-local.sh --force-update` (prod image is unaffected — fresh install).
- ✅ ~~SMTP env inconsistency~~ (fixed in ADR-0010: all routes read `SMTP_PASSWORD`).
- ✅ **One deployment story (ADR-0022):** self-hosted docker-compose on a single VM. k8s/, kubectl scripts, and the Vercel workflow are **deleted**. `Dockerfile.prod` verified: image builds (595 MB), boots healthy (`/api/health` → healthy in 368 ms), and migrates offline via the self-contained `/app/prisma-cli` (the CLI's ~33-package closure — e.g. `effect` — is never standalone-traced). `docker/nginx/*` now exists (TLS/ACME/websocket/SSE tuning); prod compose: no DB host ports, `migrate` one-shot, certbot, monitoring behind a profile (ADR-0023). Module-scope OpenAI constructors made lazy (3 libs) — they crashed hermetic key-less builds.

### Documentation map (which file to trust for what)
- ✅ **Consolidated (ADR-0024, 2026-07-06):** `README.md` slimmed 2,722→125 lines (overview + setup + links); `TODO.md` reconciled to code reality with per-ADR cross-references (the one open-work list); `TODO_HELP_DASHBOARD.md` collapsed to a 35-line historical pointer. Canonical status = this file's "Current State" + `ADR/`.
- `AI_MOCK_DATA.md` — historical audit doc; ~~its Priority-1 fix was never applied~~ — **RESOLVED (ADR-0018, 2026-07-07): `simpleAIService` deleted, both bypass routes on `aiService`.** ~~All six social providers still return `generateMockAnalytics()`~~ — **RESOLVED (ADR-0009).**
- `STRUCTURE.md` (60KB page-by-page map) is current-ish; stale duplicate `STRUCTURES.md` deleted 2026-07-03.
...
✅ **Swept (ADR-0024, 2026-07-06):** everything in the audit inventory is deleted — dead lib modules, `lazy-components` + its 6 dead component dependents, dead post-composer twin, `page.tsx.backup`/`simple.tsx`/`/test`/`/about` (fictional bios)/`/dashboard/showcase`/`/dashboard/customers`, the fake API-versioning layer (`posts/v1|v2`, `/api/version`, `api-docs`), the schema-broken mock admin video suite, the mock FAQ auto-generate endpoint+panel, `prisma/seed.js`, committed `logs/` (untracked + gitignored). Earlier ADR strikes: NotificationCenter mounted (0010), jobs runtime wired (0008), websocket/fastify deleted (0010), `BrandingConfiguration` dropped (0016), i18n cut to `en` + mock workspace switcher deleted (0017). **Still intentionally kept:** `src/components/ai/*` + `src/components/audience/*` (unmounted — ADR-0018 will mount them). **Keep-it-clean gate:** `npm run knip` (knip.config.ts; honest 33-unused-file baseline, non-blocking for one sprint) — its findings are the next hygiene round's work list.
