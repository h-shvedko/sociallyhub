...
| ADR | Title | Status | Decision in one line |
|---|---|---|---|
| [0004](ADR-0004-platform-authorization-model.md) | Platform Authorization Model & RBAC Enforcement | Proposed | Two-tier model: new `User.isPlatformAdmin` gates `/api/admin/**` and global settings; `WorkspaceRole` enforced only via central helpers; drop the never-enforced Role/UserRole/Permission tables. |
